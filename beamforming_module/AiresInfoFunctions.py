...
def GetEffectiveRefractionIndex(x0,y0,z0, ns=325, kr=-0.1218, zant=0, xant=0, yant=0, stepsize = 10000): #NOTE THAT THE ORDER OF ANTENNA POSITIONS IS ZANT,XANT,YANT (historic compatibility reasons)
...
#Vectorised version of GetEffectiveRefractionIndex
def ZHSEffectiveRefractionIndexvectTwice(X0, Xa, ns=325,kr=-0.1218,stepsize = 10000):
    R_earth = 6370949.0

    R02 = X0[:,0]*X0[:,0] + X0[:,1]*X0[:,1]

    # Altitude of emission in km
    h0 = (np.sqrt( (X0[:,2]+R_earth)*(X0[:,2]+R_earth) + R02 ) - R_earth)/1e3 #Shape N_sources
    # print('Altitude of emission in km = ',h0)
    # print(h0)

    # Refractivity at emission points
    rh0 = ns*np.exp(kr*h0)

    modr = np.sqrt(R02)
    # print(modr)
    avn = np.zeros((X0.shape[0], Xa.shape[0]))  #Shape N_sources x N_ants
    n_eff = np.zeros((X0.shape[0], Xa.shape[0]))  #Shape N_sources x N_ants

    large_distance_mask = modr > 1e3
    U = Xa[None,:,:] - X0[large_distance_mask,None,:]   #shape N_sources_large_distance x N_ants x 3
    # Divide into pieces shorter than 10km
    nint = np.round(modr[large_distance_mask]/stepsize).astype(int)+1  #shape N_sources_large_distance
    K = U/nint[:, None, None]    #shape N_sources_large_distance x N_ants x 3
    nint_init = nint.copy()  #Shape N_sources_large_distance
    #Current point coordinate
    Curr = np.repeat(X0[large_distance_mask, None, :], Xa.shape[0], axis=1) # Shape N_sources_large_distance x Nants x 3
    Currh = np.repeat(h0[large_distance_mask, None], Xa.shape[0], axis=1)   #Shape N_sources_large_distance x Nants
    sum = np.zeros_like(Currh)  #Shape N_sources_large_distance x Nants
    nint_sup_0 = nint > 0 #Shape N_sources_large_distance
    while nint_sup_0.any():
      currh_nint = Currh[nint_sup_0]
      # Next point
      Next = Curr[nint_sup_0,:,:] + K[nint_sup_0,:,:] # Shape N_sources_large_distance_with_nint x Nants x 3
      # Next horizontal distance 
      nextR2 = Next[:, :, 0]*Next[:, :, 0] + Next[:, :, 1]*Next[:, :, 1]
      # Next altitude
      nexth  = (np.sqrt( (Next[:, :, 2]+R_earth)**2 + nextR2 ) - R_earth)/1e3 #Shape N_sources_large_distance_with_nint x Nants

      mask = np.abs(nexth-currh_nint) > 1e-10  #Shape N_sources_large_distance_with_nint x Nants

      increment = np.zeros_like(nexth)  #shape N_sources_large_distance_with_nint x Nants
      increment[mask] = (np.exp(kr*nexth[mask])-np.exp(kr*currh_nint[mask]))/(kr*(nexth[mask]-currh_nint[mask]))
      increment[~mask] = np.exp(kr*currh_nint[~mask])
      sum[nint_sup_0] += increment
      Curr[nint_sup_0,:,:] = Next
      Currh[nint_sup_0] = nexth

      nint[nint_sup_0] -= 1
      nint_sup_0 = nint > 0

    avn[large_distance_mask,:] = ns*sum/nint_init[:, None]
    n_eff[large_distance_mask,:] = 1. + 1e-6*avn[large_distance_mask,:] # Effective (average) index


    # Small distance part
    small_distance_mask = ~large_distance_mask  # shape N_sources
    # Antenna altitude
    hd = Xa[:,2]/1e3  # shape Nants
    avn[small_distance_mask,:] = (ns/(kr*(hd[None, :]-h0[small_distance_mask, None])))*(np.exp(kr*hd[None, :])-np.exp(kr*h0[small_distance_mask, None]))
    n_eff[small_distance_mask,:] = 1. + 1e-6*avn[small_distance_mask,:] # Effective (average) index
    return (n_eff)


#Vectorised version of GetEffectiveRefractionIndex
def ZHSEffectiveRefractionIndexvect(X0, Xa, ns=325,kr=-0.1218,stepsize = 10000):
    R_earth = 6370949.0

    R02 = X0[0]*X0[0] + X0[1]*X0[1]

    # Altitude of emission in km
    h0 = (np.sqrt( (X0[2]+R_earth)*(X0[2]+R_earth) + R02 ) - R_earth)/1e3
    # print('Altitude of emission in km = ',h0)
    # print(h0)
    ...
    if (modr > 1e3):
        ...
        U = Xa-X0
        # Divide into pieces shorter than stepsize
...
def GetZHSEffectiveRefractionIndex(x0,y0,z0,xant=0,yant=0,zant=0,ns=325,kr=-0.1218,stepsize = 10000):
