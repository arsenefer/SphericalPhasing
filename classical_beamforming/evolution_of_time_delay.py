...
def plot_event(signals, antenna_pos, xmax_pos, tau_pos, sampling_points=None):
    ...
    axs[0].scatter(xmax_pos[0], xmax_pos[1], marker='*', c='r', s=100)
    axs[0].scatter(tau_pos[0], tau_pos[1], marker='*', c='g', s=70)
    ...
    axs[1].scatter(xmax_pos[1], xmax_pos[2], marker='*', c='r', s=100)
...
def plot_time_delay(xant, yant, zant, signals, samplepoints, layout_dir, ref_ant=None, fig=None, ax=None):
    ...
    ax.scatter(
        ...
        relative_time_delays, marker='s', s=10, c=np.repeat(amps[None,:], len(samplepoints), axis=0), cmap='viridis')
    # for ant_idx in range(len(xant)):
    ax.set_xlabel('Distance from $X{\\rm max}$ along layout direction (m)')
...
def plot_amps(samplepoints, list_summed_signals, method='amplitude',fig=None, ax=None, label=None):
    ...
    ax.set_xlabel('Distance from $X{\\rm max}$ along chosen direction (m)')
...
def main(config: Config):
    ...
    Eventlist = range(0, 20)  # List of events to process
    ...
    for i, eventi in enumerate(Eventlist):
        ...
        amps = np.max(np.linalg.norm(signals[1:, :, :], axis=0), axis=-1)
        which_ants = np.where(amps > (0 * np.max(amps)))[0]
        ref_ant = np.argmax(amps[which_ants])

        layout_dir = np.average(antenna_pos, weights=amps**2, axis=0) - xmax_pos
        layout_dir = layout_dir / np.linalg.norm(layout_dir)
        samplepoints_along_layout = np.linspace(xmax_pos + layout_dir * 3000, xmax_pos - layout_dir * 3000, 100+1, endpoint=True)

        samplepoints_along_shower = np.linspace(xmax_pos + shower_dir * 3000, xmax_pos - shower_dir * 3000, 100+1, endpoint=True)
        ...
        plot_event(signals[:,which_ants], antenna_pos[which_ants], xmax_pos, tau_pos, sampling_points=samplepoints_along_layout)
        plot_time_delay(xant[which_ants], yant[which_ants], zant[which_ants], signals[:,which_ants], samplepoints_along_layout, layout_dir, ref_ant=ref_ant, fig=fig, ax=axs[0,0])
        plot_time_delay(xant[which_ants], yant[which_ants], zant[which_ants], signals[:,which_ants], samplepoints_along_shower, shower_dir, ref_ant=ref_ant, fig=fig, ax=axs[0,1])
        ...
        plot_amps(samplepoints_along_layout, list_summed_signals_layout, method='amplitude', ax = axs[1,0], label='Along layout direction')
        plot_amps(samplepoints_along_shower, list_summed_signals_shower, method='amplitude', ax = axs[1,1], label='Along shower direction')
        
        axs[0,1].set_xlabel('Distance from $X{\\rm max}$ along shower axis (m)')
